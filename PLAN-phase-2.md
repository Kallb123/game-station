...
| Is `dart test -p chrome` available on the CI runner without extra setup? | Assumed yes — `ubuntu-latest` ships Chrome. | PR 1's first CI run. If not, the step becomes a documented gap in §7's masking risk rather than a silent omission. |
