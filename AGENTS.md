...
Goldens are regenerated by `cd packages/puzzle_engine && dart run tool/regen_goldens.dart`, and only
when the change to generation is meant to change output. Nothing stops that command being run to turn
a red build green, which is why the diff it produces names every puzzle that moved: reading it is the
review, and an unexplained one is the finding.
